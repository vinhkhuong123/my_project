  ...
  const [productDetails, setProductDetails] = useState();
    ...
    useEffect(() => {
        const fetchProductData = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/products/${editingProductId}`);
                if (response.data.status) {
                    setProductDetails(response.data.data.data[0]);
                } else {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(errors).length > 0) {
            ...
            try {
                const response = await axios.put(`http://localhost:8000/products/${editingProductId}`, productDetails);
